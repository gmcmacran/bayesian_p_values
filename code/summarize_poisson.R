...
sim_df %>%
  ...
  ggplot(aes(x = factor(theta), y = pvalue)) +
  geom_boxplot() +
  scale_y_continuous(breaks = seq(0, 1, .20), labels = percent, limits = c(0, 1)) +
  facet_grid(rows = vars(seed), cols = vars(N)) +
  labs(title = "Alternative: Less", x = "Lambda", y = "P Value")

ggsave(filename = "results/graphs/poisson_less.png", width = 10, height = 10)
...
sim_df %>%
  ...
  scale_y_continuous(breaks = seq(0, 1, .20), labels = percent, limits = c(0, 1)) +
  facet_grid(rows = vars(seed), cols = vars(N)) +
  labs(title = "Alternative: Greater", x = "Lambda", y = "P Value")

ggsave(filename = "results/graphs/poisson_greater.png", width = 10, height = 10)
...
temp <- sim_df %>%
  group_by(theta, N, alt, alpha, beta) %>%
...
ggsave(filename = "results/graphs/poisson_prior_params.png", width = 10, height = 10)
