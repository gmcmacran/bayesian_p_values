...
sim_df %>%
  ...
  ggplot(aes(x = factor(p), y = pvalue)) +
  geom_boxplot() +
  scale_y_continuous(breaks = seq(0, 1, .20), labels = percent, limits = c(0, 1)) +
  facet_grid(rows = vars(seed), cols = vars(N)) +
  labs(title = "Alternative: Less", x = "P", y = "P Value")

ggsave(filename = "results/graphs/binomial_less.png", width = 10, height = 10)
...
sim_df %>%
  ...
  scale_y_continuous(breaks = seq(0, 1, .20), labels = percent, limits = c(0, 1)) +
  facet_grid(rows = vars(seed), cols = vars(N)) +
  labs(title = "Alternative: Greater", x = "P", y = "P Value")

ggsave(filename = "results/graphs/binomial_greater.png", width = 10, height = 10)
...
ggsave(filename = "results/graphs/binomial_prior_params.png", width = 10, height = 10)
